...
docs:
	...
	nvim --headless -u NONE \
		...
		-c 'lua require("mini.doc").generate({ "lua/myplugin/init.lua" }, "doc/myplugin.txt")' \
		-c 'lua local p="doc/myplugin.txt"; local v=(vim.fn.readfile("VERSION")[1] or ""):gsub("%s+$$",""); local out={}; for _,ln in ipairs(vim.fn.readfile(p)) do if not ln:match("^Version:%s") then out[#out+1]=ln end end; table.insert(out, 4, "Version: " .. v); table.insert(out, 5, ""); vim.fn.writefile(out, p)' \
		-c 'qa'
	@rm -f doc/tags
